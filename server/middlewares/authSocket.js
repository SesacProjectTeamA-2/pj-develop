...
const authSocketUtil = {
  checkToken: async (socket, next) => {
    try {
      ...
      // 유효기간 만료
      if (user === jwtUtil.TOKEN_EXPIRED)
        return next(new Error('유효기간 만료된 토큰'));

      // 유효하지 않는 토큰
      if (user === jwtUtil.TOKEN_INVALID)
        return next(new Error('유효하지 않는 토큰'));

      // 토근 : undefined
      if (user.uSeq === undefined) return next(new Error('토큰 : undefined'));
